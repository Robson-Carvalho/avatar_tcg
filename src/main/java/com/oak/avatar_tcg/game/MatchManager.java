...
public class MatchManager {
    ...
    public String createMatch(OakRealTime socket1, String player1, OakRealTime socket2, String player2) throws Exception {
        ...
        synchronized (match) {
            if (match.getGameState().getTurnPlayerId().equals(userID)) {
                if (action.equals("activateCard")) {
                    ...
                    if (match.getGameState().getPlayerOne().getPlayedCard()
                            && match.getGameState().getPlayerTwo().getPlayedCard()) {
                        match.battle();
                    }
    ...
    public String getStateMatch(String matchId) {
        ...
        if (match == null) return "{}";
        synchronized (match) {
            return match.getGameStateJson();
        }
    }

    public OakRealTime getOpponentInMatch(String playerID, String matchID) {
        Match match = matches.get(matchID);
    ...
    public String getUserIDBySocket(OakRealTime socket) {
    ...
    public String getMatchIDBySocket(OakRealTime socket) {
